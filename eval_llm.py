...
from model.model_gugugaga import gugugagaConfig, gugugagaForCausalLM
...
def init_model(args):
    ...
    if 'model' in args.load_from:
        model = gugugagaForCausalLM(gugugagaConfig(
    ...
    print(f'gugugaga模型参数: {sum(p.numel() for p in model.parameters()) / 1e6:.2f} M(illion)')
    return model.eval().to(args.device), tokenizer

def main():
    parser = argparse.ArgumentParser(description="gugugaga模型推理与对话")
